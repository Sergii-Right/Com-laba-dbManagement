...
{
	class IDB_ASMX_FOR_WCF : IDB, IDisposable
	{
		ASMX_For_WCF_ProxyClass.ServiceSharedDatabase service = null;

		public IDB_ASMX_FOR_WCF()
		{
			service = new ASMX_For_WCF_ProxyClass.ServiceSharedDatabase();
			service.Url = "http://localhost:8888/ServiceSharedDatabase.svc";
			service.Timeout = 100 * 1000;
		}

		private COM_Laba.DatabaseData Convert(ASMX_For_WCF_ProxyClass.DatabaseData databaseData) { return new COM_Laba.DatabaseData() { Name = databaseData.Name, Tables = databaseData.Tables }; }
		private COM_Laba.TableData Convert(ASMX_For_WCF_ProxyClass.TableData tableData) { return new COM_Laba.TableData() { Name = tableData.Name, Columns = tableData.Columns, Rows = tableData.Rows }; }
		private COM_Laba.Column Convert(ASMX_For_WCF_ProxyClass.Column column) { return new COM_Laba.Column() { Name = column.Name, Type = column.Type, Values = Convert(column.Values) }; }
		private List<COM_Laba.Value> Convert(ASMX_For_WCF_ProxyClass.Value[] values) { var res = new List<COM_Laba.Value>(); for (int i = 0; i < values.Length; i++) res.Add(Convert(values[i])); return res; }
		private COM_Laba.Value Convert(ASMX_For_WCF_ProxyClass.Value value) { return new COM_Laba.Value() { Data = value.Data }; }
		...
		public void SetValue(string database, string table, string column, int row, string value) { service.SetValue(database, table, column, row, true, value); }
		...
		public void DeleteRow(string database, string table, int row) { service.DeleteRow(database, table, row, true); }
